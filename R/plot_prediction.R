plot_prediction<-function(list_result,panel="both",heights=c(1.5,1)){
  ...
  if(panel%in%c("both","longitudinal")){
    pushViewport(viewport(layout.pos.row=1))
    varnamesv<-c("Fast. glu.\n(log scale)", "2-hour glu.\n(log scale)", "Fast. Cp.\n(log scale)", "2-hour Cp.\n(log scale)",
                 "HbA1c", "GADA-z\n(log scale)", "IA-2A-z\n(log scale)", "IAA-z\n(log scale)", "ZnT8A-z\n(log scale)",
                 "FDR", "Age", "DQ8X", "DQ2X")
    if(panel%in%c("both"))pushViewport(plotViewport(margins=c(0,8,0,2)+0.1))
    if(panel%in%c("longitudinal"))pushViewport(plotViewport(margins=c(3,8,0,2)+0.1))
